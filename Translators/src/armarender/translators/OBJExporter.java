...
public class OBJExporter
{
  ...
  public static void writeScene(Scene theScene, PrintWriter out, boolean wholeScene, double tol, boolean smooth, boolean alwaysStoreNormals, TextureImageExporter textureExporter, String mtlFilename)
  {
    ...
    out.println("#Produced by Arma Automotive - Arma Design Studio "+ArmaRender.getVersion()+", "+(new Date()).toString());
    if (mtlFilename != null)
    ...
    for (int i = 0; i < theScene.getNumObjects(); i++){
        ...
        if(info.isChildrenHiddenWhenHidden() == true){
            continue;
        }
        ...
        for (int j = 0; j < vert.length; j++)
          {
            ...
            out.println("v "+nf.format((v.x*scale))+" "+nf.format((v.y*scale))+" "+nf.format((v.z*scale)));
          }
        if (needNormals)
          for (int j = 0; j < norm.length; j++)
            {
              if (norm[j] == null)
              ...
              else
                {
                  ...
                  out.println("vn "+nf.format((v.x*scale))+" "+nf.format((v.y*scale))+" "+nf.format((v.z*scale)));
                }
            }
