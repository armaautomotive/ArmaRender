...
public class RaytracerRenderer implements Renderer, Runnable
{
  ...
  public void configurePreview()
  {
    ...
    reducedMemory = false;
    roulette = false;
    surfaceError = ArmaRender.getPreferences().getInteractiveSurfaceError();
    giMode = GI_NONE;
  ...
  protected void buildScene()
  {
    ...
    ThreadManager threads = new ThreadManager(theScene.getNumObjects(), new ThreadManager.Task()
    {
      public void execute(int index)
      {
        ...
        if (info.isVisible() && info.isHiddenByParent() == false) { // isChildrenHiddenWhenHidden()
