...
public class TriMeshEditorWindow extends MeshEditorWindow implements EditingWindow
{
  ...
  private BMenuItem editMenuItem[], selectMenuItem[], meshMenuItem[], toolsMenuItem[], skeletonMenuItem[];
  ...
  public TriMeshEditorWindow(EditingWindow parent, String title, ObjectInfo obj, Runnable onClose, boolean allowTopology)
  {
    ...
    createMeshMenu((TriangleMesh) obj.getObject());
      createToolsMenu((TriangleMesh) obj.getObject());
  ...
  void createMeshMenu(TriangleMesh obj)
  {
    ...
    void createToolsMenu(TriangleMesh obj){
        BMenu toolsMenu = Translate.menu("Tools");
        menubar.add(toolsMenu);
        
        toolsMenuItem = new BMenuItem [10];
        toolsMenu.add(toolsMenuItem[0] = Translate.menuItem("Selection to Arc Tube Feature", this, "selectionToArcFeatureCommand"));
    }
  ...
  int [] findProjectedEdges()
  {
    ...
    divMesh = mesh.convertToTriangleMesh(ArmaRender.getPreferences().getInteractiveSurfaceError());
  ...
  public void adjustDeltas(Vec3 delta[])
  {
    ...
    /**
     * selectionToArcFeatureCommand
     *
     * Description: Generate arc tube features from selected mesh point data.
     */
    public void selectionToArcFeatureCommand(){
        if(window == null){
            return;
        }
        MeshVertex vert[] = mesh.getVertices();
        //System.out.println("mesh points: " + vert.length);
        
        //System.out.println("id: " + objInfo.getId() );
        
        CoordinateSystem editObjectCS = objInfo.getCoords().duplicate();
        //if(window != null){ // Get the scene coord system.
            ObjectInfo sceneSelectedObject = window.getScene().getObjectById(objInfo.getId());
            editObjectCS = sceneSelectedObject.getCoords().duplicate();
        //}
        
        Mat4 aMat4 = editObjectCS.duplicate().fromLocal();
        Vector<Vec3> selectedPoints = new Vector<Vec3>();
        for(int i = 0; i < selected.length; i++){
            boolean s = selected[i];
            if(s && vert.length >= i){
                MeshVertex mv = vert[i];
        ...
        if(selectedPoints.size() == 0){
            System.out.println("Error: No points selected.");
            return;
        }
        
        // Organize points by distance.
        Vector<Vec3> usedPoints = new Vector<Vec3>();
        Vector<Vec3> orderedPoints = new Vector<Vec3>();
        
        for(int i = 1; i < selectedPoints.size(); i++){
            //Vec3 prev = selectedPoints
        }
        
        // Add first point.
        orderedPoints.addElement(selectedPoints.elementAt(0));
        
        while( orderedPoints.size() < selectedPoints.size() ){
            Vec3 lastOrderedPoint = orderedPoints.elementAt(orderedPoints.size() - 1);
            Vec3 closestPoint = null;
            double closestDist = 99999;
            for(int i = 0; i < selectedPoints.size(); i++){
                Vec3 currPoint = selectedPoints.elementAt(i);
                if(orderedPoints.contains(currPoint) == false){
                    double distance = lastOrderedPoint.distance( currPoint );
            ...
            if(closestPoint != null){
                orderedPoints.addElement(closestPoint);
            }
        }
        
        Curve cutProfileCurve = Curve.getCurveFromPoints(orderedPoints);
        cutProfileCurve.setSmoothingMethod(Mesh.NO_SMOOTHING);
        cutProfileCurve.setClosed(true);
        CoordinateSystem coords = new CoordinateSystem(new Vec3(0.0, 0.0, 0.0), new Vec3(0.0, 0.0, 1.0), new Vec3(0.0, 1.0, 0.0));
        ObjectInfo cutCurveObj = new ObjectInfo(cutProfileCurve, coords, "Notch");
        
        // Find Arc Tube object to add new feature to.
        boolean addedNotchToArc = false;
        if(window != null && selectedPoints.size() > 0){
            Scene scene = window.getScene();
            Vector<ObjectInfo> sceneObjects = scene.getObjects();
            for(int i = 0; i < sceneObjects.size(); i++){
                ObjectInfo curr = sceneObjects.elementAt(i);
                if(curr.getObject() instanceof ArcObject){
                    BoundingBox currBounds = curr.getTranslatedBounds();
                    double margin = ((ArcObject)curr.getObject()).getTubeDiameter();
                    currBounds.expandMargin(margin);
                    Vec3 selectedPoint = selectedPoints.elementAt(0);
                    if( currBounds.contains(selectedPoint) ){
                        //System.out.println("Found arc to add notch to: " + curr.getName() + " - " + curr.getObject().getClass().getName() );
                        
                        // Generate new curve cut profile object.
                        
                        ObjectInfo currSceneObject = window.getScene().getObjectById(curr.getId());
                        
                        UndoRecord undo = new UndoRecord(window, false);
                        window.addObject(cutCurveObj, undo);
                        
                        cutCurveObj.setParent(currSceneObject);
                        currSceneObject.addChild(cutCurveObj, 0);
                        
                        window.updateImage();
                        window.updateTree();
                        
                        addedNotchToArc = true;
                    }
                }
            }
            //System.out.println(" sceneObjects.size(): " + sceneObjects.size());
        }
        // Arc tube not found, add notch to scene anyway.
        if(addedNotchToArc == false){ // Add notch to scene
            if(window != null){
                UndoRecord undo = new UndoRecord(window, false);
                window.addObject(cutCurveObj, undo);
                window.updateImage();
