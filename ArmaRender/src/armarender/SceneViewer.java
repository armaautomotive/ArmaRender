...
public class SceneViewer extends ViewerCanvas
{
  ...
  {
    super(ArmaRender.getPreferences().getUseOpenGL() && isOpenGLAvailable() && !forceSoftwareRendering);
  ...
  protected void mousePressed(WidgetMouseEvent e)
  {
    ...
    for (i = 0; i < sel.length; i++)
    {
      ...
      expandBounds(bounds);
      if (!info.isLocked() && bounds != null && !info.isChildrenHiddenWhenHidden() && pointInRectangle(p, bounds))
      {
          ...
          if(info.getObject() instanceof Curve &&
             info.getObject() instanceof DimensionObject == false &&                        // don't do this for dimension objects.
          ...
          if(activeTool instanceof MoveObjectTool == false && activeTool instanceof MoveClosestObjectTool == false){
      ...
      for (i = 0; i < theScene.getNumObjects(); i++)
      {
          ...
          if (info.isVisible() && !info.isLocked() && !info.isChildrenHiddenWhenHidden() ) //
          {
              ...
              if (bounds != null && pointInRectangle(p, bounds)){
                  ...
                  if(info.getObject().canConvertToTriangleMesh() != Object3D.CANT_CONVERT   // Is (or can be) Mesh
                     && info.getObject() instanceof Curve == false                          // Not Curve
    ...
    {
      info = theScene.getObject(i);
      if (info.isVisible() && !info.isLocked() && !info.isChildrenHiddenWhenHidden())           // scene obj is visible
      {
        theCamera.setObjectTransform(info.getCoords().fromLocal());
     ...
     * intersectsCurve
     *
     * Description: does a click fall on a curve expanded boundary.
     *  1) Find the 2D screen coordinates of the curve points.
     *  2)
     */
     ...
     * intersectsVector
     *
     * Description: Does a face in this object collide with a click vector.
     * NOTE: This only works in parallel projection.
     *
     */
    public boolean intersectsVector(ObjectInfo info, Vec3 a, Vec3 b){
           ...
           && info.getObject() instanceof armarender.object.CSGObject == false)
        {
            ...
            boolean meshColided = false;
            for(int f = 0; f < faces.length && meshColided == false; f++){
                ...
                if(colDist != Double.MIN_VALUE || colDist2 != Double.MIN_VALUE){ // Double.MIN_VALUE is 0
                    result = true;
                    meshColided = true;
                    f = faces.length; // break
                    //return true;
                    
                    //System.out.println(" YES colDist: " + colDist +  " colDist2: " + colDist2 );
                } else {
                    //System.out.println(" NO  colDist: " + colDist +  " colDist2: " + colDist2 + "   min " + Double.MIN_VALUE );
                }
            }
        } else {
            result = true;
        ...
        if(info.getObject() instanceof armarender.object.CSGObject){
            System.out.println("Warning: Can't accurately pick from CSGObjects. Using bounds instead of polygons.");
            ...
            RenderingTriangle triangle[] = csg.triangle;
            for (i = 0; i < triangle.length; i++)
            {
                Vec3 v1 = csg.vert[triangle[i].v1];
                Vec3 v2 = csg.vert[triangle[i].v2];
                Vec3 v3 = csg.vert[triangle[i].v3];
                System.out.println(" csg vec " + v1);
            }
            */
        ...
        Thread thread = new Thread(){
            public void run(){
                ...
                if (draggingSelectionBox)
                {
                  ...
                  {
                    info = theScene.getObject(i);
                    if (info.isVisible() && !info.isLocked() && !info.isChildrenHiddenWhenHidden() && (dragRegionX != 0 && dragRegionY != 0)) 
                    {
                      if(i % 10 == 0){
                          double progress = ((double)i / (double)theScene.getNumObjects()) * (double)100;
                          //System.out.println("% " + progress);
                          ...
                          if(currTime > startTime + 1000 && m.isVisible() == false){ // if selection is taking time, display progress bar.
                              m.setVisible(true);
                          }
                      }
                      theCamera.setObjectTransform(info.getCoords().fromLocal());
