...
public class ConcaveHull {
    ...
    // Replace JavaJF Pair object.
    public class Pair {
        public Double key;
        public Point value;
        public Pair(Double key, Point value){
            this.key = key;
            this.value = value;
        }
        ...
        public boolean equals(Object o){
            if(o instanceof ConcaveHull.Pair){
                Pair comparePair = (Pair)o;
                if(comparePair.key == this.key && comparePair.value.getX() == value.getX() &&  comparePair.value.getY() == value.getY()){
                    return true;
                }
            }
            return false;
        ...
        public int hashCode(){
            return (int) ( key + value.getX() + value.getY() ) ;
        }
        public String toString(){
            return key + ":" + value;
        }
    ...
    private ArrayList<Point> kNearestNeighbors(ArrayList<Point> l, Point q, Integer k) {
        ...
        for (Point o : l) {
            Pair pair = new Pair(euclideanDistance(q, o), o);
            nearestList.add(pair);
        }

        Collections.sort(nearestList, new Comparator<Pair>() { // <Double, Point>
            @Override
            public int compare(Pair o1, Pair o2) { // <Double, Point>  <Double, Point>
    ...
    private ArrayList<Point> sortByAngle(ArrayList<Point> l, Point q, Double a) {
        ...
        Collections.sort(l, new Comparator<Point>() {           // ??? Error
            @Override
