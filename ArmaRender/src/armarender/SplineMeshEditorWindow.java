...
public class SplineMeshEditorWindow extends MeshEditorWindow implements EditingWindow
{
  ...
  public SplineMeshEditorWindow(EditingWindow parent, String title, ObjectInfo obj, Runnable onClose, boolean allowTopology)
  {
    ...
    topology = allowTopology;
      ...
      this.layoutWindow = (LayoutWindow)parent;
  ...
  void createMeshMenu(SplineMesh obj)
  {
    ...
    if (topology)
      ...
      meshMenu.addSeparator();
      meshMenu.add(meshMenuItem[8] = Translate.menuItem("Create Curve from Selected Points", this, "createCurveCommand"));
  ...
  public void updateMenus()
  {
    ...
    if (count > 0)
    {
      ...
      meshMenuItem[7].setEnabled(curvemode);
        meshMenuItem[8].setEnabled(true); // create curve
    }
    else
    {
  ...
  protected void doOk()
  {
    ...
    if (onClose != null)
      ...
      if(layoutWindow != null && layoutWindow.cloth != null){
          layoutWindow.cloth.setUpdatedGeometry(objInfo);
      }
  }
  ...
  void setSmoothnessCommand()
  {
     ...
     * createCurveCommand
     *
     * Description: create a curve object from the selected points in a mesh.
     */
    void createCurveCommand(){
        //System.out.println("createCurveCommand " + objInfo.getId());
        
        ObjectInfo sceneOI = scene.getObjectById(objInfo.getId());
        
        // ***
        
        SplineMesh theMesh = (SplineMesh) objInfo.getObject();
        MeshVertex v[] = theMesh.getVertices();
        boolean curvemode = (selectMode == EDGE_MODE);
        MeshViewer view = (MeshViewer) theView[currentView];
        int count = 0;
        CoordinateSystem c;
        c = sceneOI.getCoords().duplicate(); // sceneOI objInfo
        Mat4 mat4 = c.duplicate().fromLocal();
        Vector curvePoints = new Vector();
        for (int i = 0; i < selected.length; i++) {
          //System.out.println(" sel " + i + " - " + selected[i] );
            if(selected[i]){
                Vec3 vec = new Vec3(v[i].r);
                mat4.transform(vec);
                //System.out.println("   - " + vec.x + "  " + vec.y + " " + vec.z );
                curvePoints.addElement(vec);
            }
        }
             
        
        Curve tempCurve = getCurve(curvePoints);
        ObjectInfo testCurveInfo = new ObjectInfo(tempCurve, new CoordinateSystem(), "curve");
        objInfo.addChild(testCurveInfo, 0);
        testCurveInfo.setParent(objInfo);
    ...
    public Curve getCurve(Vector<Vec3> points){
        ...
        Curve curve = new Curve(pointArray, smooths, Mesh.APPROXIMATING, false); // INTERPOLATING  APPROXIMATING
        return curve;
    }
  ...
  public void adjustDeltas(Vec3 delta[])
  {
     ...
     *
     * Description: Modify selection of points so that the middle are spaced evenly between the outer.
     */
    public void equalizeSelectionCommand(){
        hideInfographic();
        if(getObject().getObject() instanceof Mesh){
            ObjectInfo info = getObject();
            Mesh theMesh = (Mesh)info.getObject();
            MeshVertex v[] = ((Mesh)theMesh).getVertices();
            Vec3[] points = new Vec3[v.length];
            for (int i = 0; i < points.length; i++){
                points[i] = v[i].r;
            }
            Vector<Integer> selectedIndexList = new Vector<Integer>();
            for (int i = 0; i < selected.length; i++){
                if(selected[i]){
                    selectedIndexList.addElement(i);
                }
            }
            int firstIndex = 999999;
            int lastIndex = -1;
            for (int i = 0; i < selectedIndexList.size(); i++){
                int currIndex = selectedIndexList.elementAt(i);
            ...
            if(lastIndex > -1){
                Vec3 firstPoint = v[firstIndex].r;
                Vec3 lastPoint = v[lastIndex].r;
                double selectionDistance = firstPoint.distance(lastPoint);
                double equalDist = selectionDistance / selectedIndexList.size();
                for(int i = 1;  i < selectedIndexList.size() - 1; i++){
                    Vec3 currPoint = v[selectedIndexList.elementAt(i)].r;
                    double targetDist = equalDist * i;
                    double scale = (double)i / (double)(selectedIndexList.size() - 1);
                    Vec3 equalizedPoint = new Vec3(lastPoint);
                    equalizedPoint.subtract(firstPoint);
                    equalizedPoint.scale( scale );
                    equalizedPoint.add(firstPoint);
                    points[selectedIndexList.elementAt(i)] = equalizedPoint;
                }
            }
            theMesh.setVertexPositions(points);
            info.setObject((Object3D)theMesh);
