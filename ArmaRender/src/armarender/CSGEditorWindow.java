...
public class CSGEditorWindow extends ObjectEditorWindow
{
  ...
  public CSGEditorWindow(EditingWindow parent, String title, CSGObject obj, Runnable onClose)
  {
    ...
    this.onClose = onClose;
      ...
      // addObject(Object3D obj, CoordinateSystem coords, String name, UndoRecord undo)
      // int addObjectI(ObjectInfo info, UndoRecord undo)
      // public void addObject(ObjectInfo info, UndoRecord undo)
      //ObjectInfo a = new ObjectInfo
    ...
    //theScene.addObject(obj.getObject2().getObject().duplicate(), obj.getObject2().getCoords().duplicate(), obj.getObject2().getName(), null);
      ...
      theScene.addObject(obj.getObject1().duplicate(), null);
      theScene.getObjects().elementAt(theScene.getObjects().size() - 1).setId(obj.getObject1().getId());
      
      theScene.addObject(obj.getObject2().duplicate(), null);
      theScene.getObjects().elementAt(theScene.getObjects().size() - 1).setId(obj.getObject2().getId());
      
      //System.out.println(" ------ " + theScene.getObjects().elementAt(theScene.getObjects().size() - 1).getId() );
      
      for(int i = 0; i < obj.getAdditionalObjects().size(); i++){
          ObjectInfo additionalOI = obj.getAdditionalObjects().elementAt(i);
          
          CoordinateSystem cs_ = additionalOI.getCoords().duplicate();
          if(obj.cs != null){
              cs_.getOrigin().subtract( obj.cs.getOrigin() );
          }
          
          ObjectInfo addoi = new ObjectInfo(additionalOI.getObject().duplicate(), cs_, additionalOI.getName());
          addoi.setId(additionalOI.getId());
          //System.out.println("additionalOI id " + additionalOI.getId() );
          //theScene.addObject(additionalOI.getObject().duplicate(),
          //                   cs_, // additionalOI.getCoords().duplicate(),
          //                   additionalOI.getName(), null);
          
          theScene.addObject(addoi, null); // Scene.addObject() overwrites the id, we have to reset it.
          addoi.setId(additionalOI.getId());
      ...
      for(int j = 0; j < sceneObjects.size(); j++){
            ObjectInfo sceneObject = sceneObjects.elementAt(j);
          System.out.println("  DEBUG *** " + sceneObject.getId() );   // WRONG!!!
      }
  ...
  {
    return new SceneViewer(index, theScene, controls, this);
  }
  ...
  protected void createViewMenu()
  {
    ...
    if (ArmaRender.getPreferences().getObjectPreviewRenderer() != null)
    {
  ...
  {
      System.out.println("updateFromScene() ");
    theObject.setComponentObjects(theScene.getObject(0), theScene.getObject(1));
      ...
      for(int i = 0; i < theObject.getAdditionalObjects().size(); i++){
          ...
          //Object3D additional3D = (CSGObject)additionalOI.getObject();
          int additionalID = additionalOI.getId();
          
          for(int j = 0; j < sceneObjects.size(); j++){
              ObjectInfo sceneObject = sceneObjects.elementAt(j);
              
              //System.out.println("sceneObject.getId(): " + sceneObject.getId()+ "   additionalID: " +  additionalID);
              if(sceneObject.getId() == additionalID){
                  
                  // additionalOI vs. sceneObject
                  System.out.println( " coords  " + additionalOI.getCoords().getOrigin()  +"  sceneObject "  + sceneObject.getCoords().getOrigin() );
                  
                  
                  theObject.getAdditionalObjects().setElementAt( sceneObject, i );
                  
                  System.out.println(" Update additional object from scene.  " );
              }
          }
      }
  }
