...
public class ObjectTreeElement extends TreeElement
{
    ...
    private static Icon folderIcon = ThemeManager.getIcon("folder");
    private static Icon folderUnionIcon = ThemeManager.getIcon("folder_union");
    private static Icon folderSubtractIcon = ThemeManager.getIcon("folder_subtract");
    private static Icon folderIntersectionIcon = ThemeManager.getIcon("folder_intersection");
  ...
  public Icon getVisibilityIcon(){
    ...
    public Icon getFolderIcon(){
        if(info.getObject() instanceof Folder){
            Folder f = (Folder)info.getObject();
            if(f.getAction() == 1){
                return folderUnionIcon;
            } else if(f.getAction() == 2){
                return folderSubtractIcon;
            } else if(f.getAction() == 3){
                return folderIntersectionIcon;
            } else {
                return folderIcon;
            }
        }
