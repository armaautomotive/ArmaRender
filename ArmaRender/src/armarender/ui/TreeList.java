...
public class TreeList extends CustomWidget
{
  ...
  public void setSelected(Object obj, boolean selected)
  {
     ...
     * setSelectedFast
     *
     * Description: Add a selection item but don't rebuild the list as we will be adding more selection items first.
     */
    public void setSelectedFast(Object obj, boolean selected)
    {
        TreeElement el = (obj instanceof TreeElement ? (TreeElement) obj : findElement(obj));
        boolean wasDisabled = updateDisabled;
        updateDisabled = true;
   ...
   * Paint the tree.
   *
   * Description: Draw the item list elements.
   */
  private void paint(RepaintEvent ev)
  {
    ...
    Color black = new Color(70, 70, 70); // Color.black
    ...
    for (int i = 0; i < showing.size(); i++)
    {
      ...
      Icon icon = el.getIcon(); // lock icon
      if (icon != null)
      {
        ...
        // folder icon
        Icon folderIcon = el.getFolderIcon();
        if (folderIcon != null){
            folderIcon.paintIcon(getComponent(), g, x, y + 2); //
            x += folderIcon.getIconWidth();
            x += 2;
        }
        
      if (el.isSelected())
      {
        g.setColor(el.isGray() ? Color.gray : black); // black Color.black
        //g.fillRect(x, y, dim.width-x, rowHeight-3); // rectangle
          int cornerRadius = 2;
          g.fillRoundRect(x, y, dim.width-x, rowHeight-3, cornerRadius, cornerRadius); // rounded corners.
        g.setColor(Color.white);
        g.drawString(el.getLabel(), x+1, y+fm.getMaxAscent());
      }
      else
      {
        g.setColor(el.isGray() ? Color.gray : black); // Color.black
        g.drawString(el.getLabel(), x+1, y+fm.getMaxAscent());
      }
  ...
  private void mousePressed(MousePressedEvent ev)
  {
    ...
    if (i == ind && el.getNumChildren() > 0)
    {
      ...
      if(pos.x > ((ind+1)*INDENT_WIDTH) + 1 && pos.x < ((ind+1)*INDENT_WIDTH) + 9){
          ...
          if( obj instanceof armarender.object.ObjectInfo ){
              ...
              MirrorMesh mirrorMesh = new MirrorMesh();
              mirrorMesh.updateVisibility(oi);
