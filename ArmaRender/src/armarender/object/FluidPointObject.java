...
//public class FluidPointObject extends Object3D implements Mesh
public class FluidPointObject extends Curve implements Mesh
{
    ...
    public Vec3 getLocation(){
        return this.getVertexPositions()[0];
    }
    
    public void setResetLocation(Vec3 resetVec[]){
        this.resetLocation = resetVec;
    }
    
    public Vec3[] getResetLocation(){
    ...
    public void resetLocation(){
        ...
        //vertex = new MeshVertex[resetLocation.length];
        for(int i = 0; i < resetLocation.length; i++){
            vertex[i] = new MeshVertex(resetLocation[i]);
        }
        //this.vertex = this.resetVertex;
    }
    
    public FluidPointObject(Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed)
    {
    ...
    public Object3D duplicate()
    {
        ...
        return new FluidPointObject(v, s, smoothingMethod, closed);
    }
    
    public void copyObject(Object3D obj)
    {
        FluidPointObject cv = (FluidPointObject) obj;
    ...
    public WireframeMesh getWireframeMesh()
    {
        ...
        if (smoothingMethod == NO_SMOOTHING)
            ...
            subdiv = subdivideFluidPoint().subdivideFluidPoint();
    ...
    public FluidPointObject subdivideFluidPoint()
    {
        //System.out.println("  FluidPointyObject.subdivideFluidPoint() ");
        if (vertex.length < 2)
            return (FluidPointObject) duplicate();
        if (vertex.length == 2)
        {
            ...
            return new FluidPointObject(newpos, news, smoothingMethod, closed);
        }
        Vec3 v[] = new Vec3 [vertex.length];
        for (int i = 0; i < v.length; i++)
            v[i] = new Vec3(vertex[i].r);
    ...
    /** Return a new FluidPoint object which has been subdivided the specified number of times to give a finer approximation of
     the curve shape. */
    
    public FluidPointObject subdivideFluidPoint(int times)
    {
        FluidPointObject c = this;
        for (int i = 0; i < times; i++)
            c = c.subdivideFluidPoint();
        return c;
    }
    
    /** The following two routines are used by subdivideFluidPoint to calculate new point positions
    ...
    {
        TriangleMesh mesh = triangulateFluidPoint();
        if (mesh != null)
            mesh = TriangleMesh.optimizeMesh(mesh);
        return mesh;
    }
    
    private TriangleMesh triangulateFluidPoint()
    {
        //System.out.println("  FluidPointObject.triangulateFluidPoint() ");
    ...
    public FluidPointObject(DataInputStream in, Scene theScene) throws IOException, InvalidObjectException
    {
        super(in, theScene);
        
        //System.out.println("  FluidPointObject( instream, scene ) ");
    ...
    public void renderObject(ObjectInfo obj, ViewerCanvas canvas, Vec3 viewDir)
    {
        ...
        if (mesh != null)
        {
            ...
            canvas.renderFluidPointObject( obj, theCamera );
    ...
    // We don't write fluid points to disk
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException
    {
        ...
        //System.out.println("  FluidPointObject writeToFile ( outstream, scene ) ");
    ...
    {
        return new FluidPointKeyframe(this);
    }
    ...
    {
        FluidPointKeyframe key = (FluidPointKeyframe) k;
    ...
    public static class FluidPointKeyframe extends MeshGesture
    {
        Vec3 vertPos[];
        float vertSmoothness[];
        FluidPointObject curve;
        
        public FluidPointKeyframe(FluidPointObject curve)
        {
            this.curve = curve;
        ...
        {
            FluidPointKeyframe k = new FluidPointKeyframe();
            if (owner instanceof FluidPointObject)
                k.curve = (FluidPointObject) owner;
            else
                k.curve = (FluidPointObject) ((ObjectInfo) owner).getObject();
        ...
        public void blendSurface(MeshGesture average, MeshGesture p[], double weight[])
        {
            ...
            FluidPointKeyframe avg = (FluidPointKeyframe) average;
            for (int i = 0; i < weight.length; i++)
            {
                FluidPointKeyframe key = (FluidPointKeyframe) p[i];
        ...
        {
            if (!(k instanceof FluidPointKeyframe))
                return false;
            FluidPointKeyframe key = (FluidPointKeyframe) k;
            for (int i = 0; i < vertPos.length; i++)
            {
        ...
        public FluidPointKeyframe(DataInputStream in, Object parent) throws IOException, InvalidObjectException
        {
            this();
            short version = in.readShort();
            if (version != 0)
                throw new InvalidObjectException("");
            curve = (FluidPointObject) parent;
