...
public class DimensionLinearObject extends Curve implements Mesh
{
    ...
    public DimensionLinearObject(Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed)
    {
    ...
    public Object3D duplicate()
    {
        ...
        return new DimensionLinearObject(v, s, smoothingMethod, closed);
    }
    
    public void copyObject(Object3D obj)
    {
        DimensionLinearObject cv = (DimensionLinearObject) obj;
    ...
    public WireframeMesh getWireframeMesh()
    {
        ...
        if (smoothingMethod == NO_SMOOTHING)
            ...
            subdiv = subdivideDimension().subdivideDimension();
    ...
    public DimensionLinearObject subdivideDimension()
    {
        //System.out.println("  DimensionLinearObject.subdivideDimension() ");
        if (vertex.length < 2)
            return (DimensionLinearObject) duplicate();
        if (vertex.length == 2)
        {
            ...
            return new DimensionLinearObject(newpos, news, smoothingMethod, closed);
        }
        Vec3 v[] = new Vec3 [vertex.length];
        for (int i = 0; i < v.length; i++)
            v[i] = new Vec3(vertex[i].r);
    ...
    public DimensionLinearObject subdivideDimension(int times)
    {
        DimensionLinearObject c = this;
        for (int i = 0; i < times; i++)
            c = c.subdivideDimension();
        return c;
    }
    ...
    {
        TriangleMesh mesh = triangulateDimension();
        if (mesh != null)
    ...
    public DimensionLinearObject(DataInputStream in, Scene theScene) throws IOException, InvalidObjectException
    {
        super(in, theScene);
    ...
    public void renderObject(ObjectInfo obj, ViewerCanvas canvas, Vec3 viewDir)
    {
        ...
        if (mesh != null)
        {
            ...
            canvas.renderDimensionLinearObject( obj, theCamera );
