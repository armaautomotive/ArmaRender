...
//import armarender.tools.LatheDialog;
//import armarender.tools.LatheTool;
//import armarender.tools.ExtrudeDialog;
//import java.util.*;
import armarender.object.TriangleMesh.*;

// Rename to ArcCurve

public class ArcObject extends Curve implements Mesh
{
    protected MeshVertex vertex[];  // 3 vertex max. Represents Arc geometry
    ...
    protected double bendRadius = 6.0;
    ...
    private static final Property PROPERTIES[] = new Property [] {
        ...
        new Property("Bend Radius",  0, 160.0, 6.0 ),
    ...
    public ArcObject(Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed)
    {
    ...
    public Object3D duplicate()
    {
        ...
        for (int i = 0; i < vertex.length; i++)
        {
            ...
            //if( i > smoothness.length ){
            if(i >= smoothness.length ){
                System.out.println("ERROR " + smoothness.length);
            }
                s[i] = smoothness[i]; // *** null
            //} else {
            //    s[i] = smoothness[0];
            //}
        }
        
        ArcObject result = new ArcObject(v, s, smoothingMethod, closed);
    ...
    {
        if(s.length > smoothness.length){                // If the new smoothness array is larger.
            smoothness = new float[s.length];
        }
    ...
    public WireframeMesh getWireframeMesh()
    {
        ...
        if (smoothingMethod == NO_SMOOTHING)
            ...
            subdiv = subdivideArc().subdivideArc();
    ...
    public ArcObject subdivideArc()
    {
        //System.out.println("  DimensionLinearObject.subdivideArc() ");
        if (vertex.length < 2)          // not valid arc object
            return (ArcObject) duplicate();
        if (vertex.length == 2)         // arc only has two points
        {
            Vec3 newpos[] = new Vec3 [] {new Vec3(vertex[0].r), vertex[0].r.plus(vertex[1].r).times(0.5), new Vec3(vertex[1].r)};
            float news[] = new float [] {smoothness[0], (smoothness[0]+smoothness[1])*0.5f, smoothness[1]}; // new smoothness
            return new ArcObject(newpos, news, smoothingMethod, closed);
        }
        Vec3 v[] = new Vec3 [vertex.length];
        for (int i = 0; i < v.length; i++)
            v[i] = new Vec3(vertex[i].r);
    ...
    public ArcObject subdivideArc(int times)
    {
        ArcObject c = this;
    ...
    {
        CurveEditorWindow ed = new CurveEditorWindow(parent, "Arc object '"+ info.getName() +"'", info, cb, true);
        ed.setVisible(true);
    ...
    public ArcObject(DataInputStream in, Scene theScene) throws IOException, InvalidObjectException // DimensionLinearObject
    {
        super(in, theScene);
    ...
    public double getLength(){
        double length = 0;
        Vector<Double> segments = getArcSegmentLength();
        for(int i = 0; i < segments.size(); i++){
            length += segments.elementAt(i);
        }
        return length;
    }
        ...
        for(int i = 2; i < verts.length; i++){ // && segment < verts.length
            ...
            if(i == verts.length - 1){  // last segment
                ...
                if(tubeDiameter > 0){
                    ...
                    //System.out.println("  - previousArcCircumfrance " + Math.round( previousArcCircumfrance *100.0)/100.0   );
                    //System.out.println("  - arcCircumfrance " + Math.round( arcCircumfrance *100.0)/100.0   );
    ...
    /**
     * getArcSegmentStraightOffsets
     *
     * Description: This function calculates a segment offset value to compensate the end point from a curved arc length.
     * If you are using arc lengths (triples) from getArcSegmentLength() you cant reference points on the segment because
     * they are unrelated. These offsets provide a distance between the seg lengtsh and the segment points.
     */
    public Vector<Double> getArcSegmentStraightOffsets(){
        Vector<Double> lengthVec = new Vector<Double>();
        lengthVec.addElement(Double.valueOf(0));
        ...
        for(int i = 2; i < verts.length; i++){ // && segment < verts.length
            ...
            double halfAngle = angle / 2;
            double arcCircumfrance = (2 * Math.PI * bendRadius) * (((180 - Math.toDegrees(angle)) / 360));
            //System.out.println("i: " + i + " arcCircumfrance: " + arcCircumfrance + " "  );
            ...
            double bendCirc = circumference * (angle / (Math.PI * 2));
            
            System.out.println("i: " + i + " bendCirc: " + bendCirc + " "  );
            */
            ...
            double distanceShift = triangleLengthC;
            ...
            double aDist = vertAL.distance(aPoint);
            double bDist = vertBL.distance(aPoint);
            //double cDist = vertA_.distance(aPoint);
            
            //System.out.println(i + " ----- aDist: " + aDist + " bDist: " + bDist + " arcCirc: " + (arcCircumfrance/2) );
            
            double segmentOffset = bDist - (arcCircumfrance/2);
            
            lengthVec.addElement(segmentOffset);
        }
    ...
    public String getObjectInfo(){
        ...
        for(int i = 2; i < verts.length; i++){
            ...
            Vec3 vertA_V = new Vec3(verts[i - 2].x, verts[i - 2].y, verts[i - 2].z);
            Vec3 vertB_V = new Vec3(verts[i - 1].x, verts[i - 1].y, verts[i - 1].z);
            Vec3 vertC_V = new Vec3(verts[i - 0].x, verts[i - 0].y, verts[i - 0].z);
            
            //vertB = new Vec3(0, 0, 0); // A and C have been moved relative to a zero B to find the angle.
            //System.out.println("            vertB: " +vertB );
            ...
            info = info + "Arc Circumfrance: " + arcCircumfrance;
    ...
    public void renderObject(ObjectInfo obj, ViewerCanvas canvas, Vec3 viewDir)
    {
        ...
        //System.out.println(" ArcObject renderObject.  Radius: " + bendRadius);
        ...
        */
        ...
        int replicateId = obj.getReplicateId();
        if(replicateId > -1 || ( obj.getParent() != null && obj.getParent().getObject() instanceof MirrorPlaneObject ) ){ // Is replicated object.
            lineColor = lightGreen; // Set colour to distinct.
            isReplicated = true;
        }
        ...
        ((ArcObject)obj.object).drawFirstPoint(canvas);
        ...
        for(int i = 2; i < verts.length; i++){
            ...
            Vec3 aLenSlide = new Vec3(0, 0.5, 0); // (arcDistance/2)
            CoordinateSystem cs;
            cs = layout.getCoords(obj);
            cs.setOrigin(new Vec3(0, 0, 0));
            //double rot[] = cs.getRotationAngles();
            //System.out.println("  XXX " +rot[0] + " " + rot[1] + " " + rot[2] );
            //c.setOrientation(-rot[0], -rot[1], -rot[2]);
            
            //double xAngle = vertBL.getAngleX(  vertAL  );
            //double yAngle = vertBL.getAngleY(  vertAL  );
            //double zAngle = vertBL.getAngleZ(  vertAL  );
            //System.out.println("xAngle: " + Math.toDegrees(xAngle) + "yAngle: " + Math.toDegrees(yAngle) + " zAngle: " + Math.toDegrees(zAngle) );
            ...
            double cbDistance = vertCL.distance(vertBL);
            //System.out.println(" --- cbDistance " + cbDistance);
            double cbScaleFactor = distanceShift / cbDistance; // (length along line)
            Vec3 cbScaledSegment = new Vec3(vertCL);
            cbScaledSegment.subtract(vertBL);
            cbScaledSegment = cbScaledSegment.times(cbScaleFactor);
            ...
            if(previousCPoint != null && previousAPoint != null){
                ...
                if(minBendSpan > 0 && previousCPoint.distance(aPoint) < minBendSpan){
                    lineColor = red;
                }
                
                canvas.renderLine(previousCPoint, aPoint, theCamera, lineColor); // a -> b-offset //  vertA_
                if(tubeDiameter > 0){
                    renderTubeCylinder(canvas, verts, previousCPoint, aPoint, isReplicated);
                }
            } else {
                
                if(minBendSpan > 0 && vertA_.distance(aPoint) < minBendSpan){
                    lineColor = red;
                }
                
                canvas.renderLine(vertA_, aPoint, theCamera, lineColor); // a -> b-offset //  vertA_
                if(tubeDiameter > 0){
                    renderTubeCylinder(canvas, verts, vertA_, aPoint, isReplicated);
                }
            }
            if(i == verts.length - 1){  // last segment
                Vec3 vertC_ = new Vec3(verts[i - 0].x, verts[i - 0].y, verts[i - 0].z);
                
                if(minBendSpan > 0 && cPoint.distance(vertC_) < minBendSpan){
                    lineColor = red;
                }
                
                canvas.renderLine(cPoint, vertC_, theCamera, lineColor);
                if(tubeDiameter > 0){
                    renderTubeCylinder(canvas, verts, cPoint, vertC_, isReplicated);
                }
            }
            ...
            canvas.renderLine(arcPoint, aPoint, theCamera, lineColor); // low res curve profile A
            canvas.renderLine(arcPoint, cPoint, theCamera, lineColor); // low res curve profile C
            
            
            //System.out.println("arcPoint: " + arcPoint);
            //System.out.println("vertB: " + vertB);
            
            
            Vec3 centrePoint = new Vec3(arcPoint); // arcCentre
            Vec3 centreDirection = new Vec3(arcPoint); centreDirection.subtract(vertB); //centreDirection.normalize();
            // translate arcCentre by bendRadius along normal vector (vertB - arcPoint)
            //System.out.println("centreDirection: " + centreDirection);
            double centreScale = (bendRadius / vertB.distance(arcPoint));
            //System.out.println("centreScale " + centreScale);
            //System.out.println(" centreDirection.x " + centreDirection.x) ;
            centrePoint.x += (centreDirection.x * centreScale);
            centrePoint.y += (centreDirection.y * centreScale);
            centrePoint.z += (centreDirection.z * centreScale);
            //System.out.println("   CENTRE POINT " + centrePoint);
            
            
            if(tubeDiameter > 0){
                // Render arc
                renderTubeTorus(canvas, vertA_V, vertB_V, vertC_V, aPoint, arcPoint, cPoint, tubeDiameter, bendRadius, angle, centrePoint, isReplicated);
                
                if(false){ // Old arc bend render from two segments.
                    renderTubeCylinder(canvas, verts, aPoint, arcPoint, isReplicated);
                    renderTubeCylinder(canvas, verts, arcPoint, cPoint, isReplicated);
                }
            }
        ...
        if(verts.length == 2){
            ...
            verts_[0] = vertA;
            verts_[1] = vertB;
            renderTubeCylinder(canvas, verts_, vertA, vertB, isReplicated);
            canvas.renderLine(vertA, vertB, theCamera, lineColor);
        }
    }
     ...
     *  @param Vec3 b -
     */
    public void renderTubeCylinder(ViewerCanvas canvas, Vec3 [] verts, Vec3 a, Vec3 b, boolean isReplicated){
        if(scene != null){
            Camera cam = canvas.getCamera();
            ...
            // <viewertransparent R="204" G="204" B="255"/>
            ...
            if(isReplicated){
                //shader = new ConstantVertexShader(new RGBColor(0.72, 0.92, 0.72));   // Replicated shader color is green.
                shader = new ConstantVertexShader(ViewerCanvas.replicatedTransparentColor);
            }
     ...
     * Description:
     *
     *  @param: Canvas - scene with object and point data.
     *  @param: Vec3 origin - Location
     *  @param: Diameter
     *  @param: Radius
     *  @param: bendAngle
     */
    public void renderTubeTorus(ViewerCanvas canvas,
                                Vec3 vertA, Vec3 vertB, Vec3 vertC,
                                Vec3 vert1, Vec3 vert2, Vec3 vert3,
                                double diameter, double bendRadius, double bendAngle,
                                Vec3 centrePoint, boolean isReplicated){
        if(scene != null){
            Camera cam = canvas.getCamera();
            Vec3 viewDir = new Vec3(0,1,0);
            ConstantVertexShader shader = new ConstantVertexShader(ViewerCanvas.transparentColor);
            if(isReplicated){
                //shader = new ConstantVertexShader(new RGBColor(0.72 , 0.92 , 0.72));   // Replicated shader color is green.
                shader = new ConstantVertexShader(ViewerCanvas.replicatedTransparentColor);
            ...
            Vec3 arcNormal = new Vec3(vertA); arcNormal.subtract(vertB); arcNormal.normalize();
            
            // Generate curve profile to lathe the torus with
            double pointRadius = diameter / 2;
            Vector curvePoints = new Vector();
            for(int i = 0; i < 12; i++){
                double pointAngle = i * 30.0;
                pointAngle = Math.toRadians(pointAngle);
                double x = 0, y = 0, z = 0;
                x = pointRadius * Math.cos(pointAngle);
                y = pointRadius * Math.sin(pointAngle);
                Vec3 point = new Vec3(x, y, z); // + vert1.x + vert1.y + vert1.z
                curvePoints.addElement(point);
            }
            // Rotate by aegment A normal
            CoordinateSystem cirleCS = new CoordinateSystem();
            cirleCS.setOrientation(arcNormal); // experimental. Sets orientation to given normal and generate a random updir normal.
            for(int i = 0; i < 12; i++){
                Vec3 p = (Vec3)curvePoints.elementAt(i);
                Mat4 mat4 = cirleCS.duplicate().fromLocal();
                mat4.transform(p);
                curvePoints.setElementAt(p, i);
            }
            // Position
            for(int i = 0; i < 12; i++){
                Vec3 p = (Vec3)curvePoints.elementAt(i);
            ...
            Curve curve = Curve.getCurveFromPoints(curvePoints);
            curve.setClosed(true);
            curve.setSmoothingMethod(Mesh.INTERPOLATING);
            ObjectInfo curveOI = new ObjectInfo(curve, new CoordinateSystem(), "ArcCurve");
            if(false){
                curve.renderObject(curveOI,  canvas, viewDir); // debug render profile segment curve
            }
            Vec3 part1 = new Vec3(vertB);
            Vec3 part2 = new Vec3(vertB);
            part1.subtract(vertA);
            part1.normalize();
            part2.subtract(vertC);
            part2.normalize();
            Vec3 latheAxis = part1.cross(part2);
            Vec3 latheRadius = part1;
            // latheAxis has to be perpendicular to A-B normal and B-C normal.
            latheAxis.normalize();
            // Lathe the circle curve into a torus.
            //int axis;
            //axis = 4; //  2;//  Z LatheTool.Y_AXIS;
            int segments = 12;
            //Vec3 torusCentre = new Vec3(-0.25, 0.0, 0.0);
            SplineMesh mesh = (SplineMesh) latheCurve(curve, segments, -(180 - Math.toDegrees(bendAngle)), bendRadius, latheAxis, latheRadius, centrePoint);
            Texture tex = canvas.getScene().getDefaultTexture();
            mesh.setTexture(tex, tex.getDefaultMapping(mesh));
            ObjectInfo torusOI = new ObjectInfo(mesh, new CoordinateSystem(), "Render Torus");
            CoordinateSystem cylCS = torusOI.getCoords();
            // Position and orient the torus to the bend location
            Vec3 torusLocation = vert2;
            RenderingMesh rm = mesh.getRenderingMesh(0.1, true, torusOI);
            Mat4 mat4 = cylCS.duplicate().fromLocal();
            rm.transformMesh(mat4);
            // Display the torus
            canvas.renderMeshTransparent(rm, shader, cam, viewDir, null);
        }
    }
     ...
     * latheCurve
     *
     * Description: Lathe a curve around a centre point.
     *  axis = updir
     *  radius = zdir
     */
    public Mesh latheCurve(Curve theCurve, int segments, double angle, double latheRadius, Vec3 _latheAxis, Vec3 _latheRadius, Vec3 torusCentre )
    {
   ...
   //     rm.transformMesh(mat4);
     ...
     * getTubeTorus
     *
     * Description: generate a torus object with geometry based on the points from intermediate segments.
     */
    public ObjectInfo getTubeTorus(
                                   Vec3 vertA, Vec3 vertB, Vec3 vertC,
                                   Vec3 vert1, Vec3 vert2, Vec3 vert3,
                                   double diameter, double bendRadius, double bendAngle,
                                   Vec3 centrePoint,
        ...
        if(scene != null){
            ...
            mesh.reverseOrientation(); // invert normals
            ObjectInfo torusOI = new ObjectInfo(mesh, new CoordinateSystem(), "Render Torus");
            
            // TODO extrude shape to make it solid
            
            
            //ExtrudeDialog extrude = new ExtrudeDialog( null );
            Vec3 normVec = new Vec3();
            //normVec.set(xField.getValue(), yField.getValue(), zField.getValue());
            double extrudeDistance = tubeDiameter / 4; // 2.5   // Thick extrudes break slicers when 3d printing.
            Curve path = null;
            Vec3 [] points = new Vec3[2]; points[0] = new Vec3(0,0,0); points[1] = new Vec3(1,0,0);
            
            path = new Curve(points, new float[0], 0, false); // Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed
            
            CoordinateSystem pathCoords;
            pathCoords = torusOI.getCoords();
            //SplineMesh sm = (SplineMesh)profile.getObject();
            
            TriangleMesh extrudedObj = (TriangleMesh)extrudeNormalsMesh(
                                                        (ObjectInfo)torusOI,                                // ObjectInfo
                                                        (SplineMesh)mesh,                                   // SplineMesh
                                                        (TriangleMesh)mesh.convertToTriangleMesh(0.05),     // TriangleMesh
                                                        (Vec3)normVec,                                      // Vec3
                                                        extrudeDistance,                                    // double
                                                        (Curve)path,                                        // Curve
                                                        (CoordinateSystem)torusOI.getCoords(),
                                                        (CoordinateSystem)pathCoords,
                                                        false);
            //extrudedObj.reverseOrientation(); // invert normals
            extrudedObj.reverseNormals();
            ObjectInfo extrudedTorusOI = new ObjectInfo(extrudedObj, new CoordinateSystem(), "Render Extruded Torus");
            torusOI = extrudedTorusOI;
        ...
        for(int i = 2; i < verts.length; i++){
            ...
            if(previousCPoint != null && previousAPoint != null){
                ...
                if(tubeDiameter > 0){
                    ...
                    // renderTubeCylinder(canvas, verts, previousCPoint, aPoint);
                    
                    ObjectInfo oi = getTubeCylinder(verts, previousCPoint, aPoint, outer);
                    result.addElement(oi);
                }
            } else {
                ...
                //canvas.renderLine(vertA_, aPoint, theCamera, lineColor); // a -> b-offset //  vertA_
                if(tubeDiameter > 0){
                    //renderTubeCylinder(canvas, verts, vertA_, aPoint);
                    
                    // renderTubeCylinder(canvas, verts, vertA_, aPoint);
                    
                    ObjectInfo oi = getTubeCylinder(verts, vertA_, aPoint, outer);
                    result.addElement(oi);
                }
            }
            if(i == verts.length - 1){  // last segment
                ...
                //canvas.renderLine(cPoint, vertC_, theCamera, lineColor);
                if(tubeDiameter > 0){
                    //renderTubeCylinder(canvas, verts, cPoint, vertC_);
                    
                    ObjectInfo oi = getTubeCylinder(verts, cPoint, vertC_, outer);
                    result.addElement(oi);
                }
            }
            ...
            if(tubeDiameter > 0){
                // Render arc
                //renderTubeTorus(canvas, vertA_V, vertB_V, vertC_V, aPoint, arcPoint, cPoint, tubeDiameter, bendRadius, angle, centrePoint);
                
                ObjectInfo oi = getTubeTorus(vertA_V, vertB_V, vertC_V, aPoint, arcPoint, cPoint, tubeDiameter, bendRadius, angle, centrePoint, tubeDiameter);
                result.addElement(oi);
                
                
                //if(false){ // Old arc bend render from two segments.
                //    renderTubeCylinder(canvas, verts, aPoint, arcPoint);
                //    renderTubeCylinder(canvas, verts, arcPoint, cPoint);
                //}
            }
    ...
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException
    {
        ...
        if(smoothness.length < vertex.length){
            System.out.println("ArcObject Error: " +smoothness.length + " " + vertex.length );
            smoothness = new float[vertex.length];
            float smooth = smoothness[0];
            for(int s = 0; s < vertex.length; s++){
                smoothness[s] = smooth;
            }
        }
    ...
    /** Extrude a triangle mesh into a solid object using vertex normal information.
        
        @param profile     the TriangleMesh to extrude
        @param path        the path along which to extrude it          Not using the path values but using the data structure. Could be cleaned up
        @param profCoords  the coordinate system of the profile
        @param pathCoords  the coordinate system of the path, Not using the path values but using the data structure. Could be cleaned up
        @param orient      if true, the orientation of the profile will follow the curve    ???
        @return the extruded object
    */
    
    public  Object3D extrudeNormalsMesh(ObjectInfo profileOI,
                                              SplineMesh splineMesh,
                                              TriangleMesh profile,
                                              Vec3 normVec,
                                              double extrudeDistance,
                                              Curve path,
                                              CoordinateSystem profCoords,
                                              CoordinateSystem pathCoords,
                                              boolean orient)
    {
      //System.out.println("extrudeNormalsMesh:");
      Vertex profVert[] = (Vertex [] ) profile.getVertices();
      MeshVertex pathVert[] = path.getVertices();                   // dep
      ...
      for (i = 0; i < pathVert.length; i++) {                                     // dep
      ...
      if (path.isClosed())
        ...
        //
        // USE splineMesh node verts normals
        // Only one segment, no angle
        
        
      // Create the extruded surface.
        t = new Vec3 [2];
        t[0] = normVec; // new Vec3(0.5, 0.5, 0.2);
        t[1] = normVec; // new Vec3(0.5, 0.5, 0.2);
        
        //System.out.println(" pathv.length " + pathv.length);
        Vec3 zdTemp = Vec3.vz(); // new Vec3(0.0, 0.0, 0.2); // zdir
        Vec3 udTemp = Vec3.vy(); // new Vec3(0.0, 0.9, 0.0); // updir  ()
      
      
      v = new Vec3 [2 * profv.length + numBoundaryPoints * (2 - 2)]; // pathv.length,   pathv.length always 2
      ...
      if (!path.isClosed())  //   // will never be closed
        {
          // Add two copies of the profile mesh, to serve as the ends.     *** this might need work ***
          
          localCoords.setOrigin(pathv[0]);
          localCoords.setOrientation(zdTemp, udTemp); //  zdir[0], updir[0]
          for (i = 0; i < profv.length; i++){                           //  FIRST FACE
            v[i] = localCoords.fromLocal().times(profv[i]);
        
          }
          k = 1; //  (pathv.length == subdiv.length ? pathv.length-1 : 2*(pathv.length-1)); //
            
          localCoords.setOrigin(pathv[pathv.length-1]);
          localCoords.setOrientation(zdTemp, udTemp); // zdir[k], updir[k]
          
          for (i = 0; i < profv.length; i++) {                              // second face
              zdTemp = Vec3.vz();
              udTemp = Vec3.vy();
              
              // Get vert normal
              MeshVertex smVerts[] = splineMesh.getVertices();
              Vec3 normals[] = splineMesh.getNormals();
              
              // Given a profv index find the closest splineMesh vert and get it's normal value.
              Vec3 profVec = new Vec3(v[i]);
              double minDistance = 99999;
              Vec3 closestNormal = new Vec3(0,0,0);
              for(int jj = 0; jj < smVerts.length; jj++){
                  Vec3 smVert = new Vec3(smVerts[jj].r);
                  double distance = smVert.distance(v[i]);
                  if(distance < minDistance){
                      minDistance = distance;
                      closestNormal = new Vec3(normals[jj]);
                  }
              }
                      
              CoordinateSystem cs = profileOI.getCoords().duplicate();
              Vec3 profileZ = cs.getZDirection();   // changes with Y totation
              Vec3 profileUp = cs.getUpDirection(); // Changes with X rotation
              
              double normalDistance = closestNormal.distance(new Vec3(0,0,0)); // closestNormal is direction // distance is extrude dir
              double normalScaleFactor = extrudeDistance * normalDistance;
              Vec3 scaledNormal = new Vec3(closestNormal);
              scaledNormal = scaledNormal.times(normalScaleFactor);
              double scaledNormalDistance = scaledNormal.distance(new Vec3(0,0,0));
              
              // Translate normal vector by orientation of selected part.
              CoordinateSystem tempLocalCoords = localCoords.duplicate();
              tempLocalCoords.transformCoordinates(cs.fromLocal()); // apply orientation from part, doesn't work
              tempLocalCoords.setOrigin(profVec); // source vec
              tempLocalCoords.setOrientation(profileZ, profileUp);
              
              v[i+profv.length] = tempLocalCoords.fromLocal().times(scaledNormal); // scaledNormal closestNormal
          }
      ...
      for (i = 0; i < pathv.length; i++)
        {
          ...
          k = 1; //  (pathv.length == subdiv.length ? i : 2*i);  // ???
          localCoords.setOrientation(zdTemp, udTemp); // zdir[k], updir[k]
         // if (angle != 0.0)
         //   {
