...
public class TriangleMesh extends Object3D implements FacetedMesh
{
  ...
  public WireframeMesh getWireframeMesh()
  {
    ...
    if (smoothingMethod == INTERPOLATING || smoothingMethod == APPROXIMATING)
      {
        ...
        for (i = 0; i < split.length; i++)
          split[i] = true;
        if (smoothingMethod == INTERPOLATING)
          mesh = subdivideButterfly(mesh, split, ArmaRender.getPreferences().getInteractiveSurfaceError());
        else
          mesh = subdivideLoop(mesh, split, ArmaRender.getPreferences().getInteractiveSurfaceError());
        v = mesh.vertex;
        e = mesh.edge;
      }
    else
    ...
    /**
     *
     * Usage: Called to render in scene and editor. Used by getPreviewMesh
     */
...
groups:     do
  ...
  */
  ...
  public static TriangleMesh subdivideLinear(TriangleMesh mesh, boolean split[])
  {
    ...
    // create arrays.
      ...
      if(edge.length > split.length){
          System.out.println("WARNING: TriangleMesh split["+split.length+"] too small. expected " + split.length + ".");
    ...
    numFace = face.length;
    for (i = 0; i < edge.length; i++)
      if (split[i]  ) // && (split.length < edge.length)
        {
          numVert++;
          numEdge += 2;
          numFace++;
    ...
    for (j = 0; j < edge.length; j++)
      if (split[j]  ) // && (split.length < edge.length)
        {
  ...
  */
  ...
  /** Get an array of normal vectors (for each vertex).  This calculates a single normal for each vertex,
  ...
  public static class TriangleMeshKeyframe extends MeshGesture
  {
    ...
    */
     ...
     */
    public Vec3 getFaceNormal(int faceIndex){
        Face [] faces = getFaces();
        Face face = faces[faceIndex];
        
        Vec3 v0 = new Vec3(vertex[face.v1].r);
        Vec3 v1 = new Vec3(vertex[face.v2].r);
        Vec3 v2 = new Vec3(vertex[face.v3].r);
