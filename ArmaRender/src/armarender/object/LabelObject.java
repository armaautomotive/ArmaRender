...
public class LabelObject extends Curve implements Mesh
{
    ...
    public LabelObject(Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed)
    {
    ...
    public Object3D duplicate()
    {
        ...
        return new LabelObject(v, s, smoothingMethod, closed);
    }
    
    public void copyObject(Object3D obj)
    {
        LabelObject cv = (LabelObject) obj;
    ...
    public WireframeMesh getWireframeMesh()
    {
        ...
        if (smoothingMethod == NO_SMOOTHING)
            ...
            subdiv = subdivideLabel().subdivideLabel();
    ...
    public LabelObject subdivideLabel()
    {
        //System.out.println("  LabelObject.subdivideLabel() ");
        if (vertex.length < 2)
            return (LabelObject) duplicate();
        if (vertex.length == 2)
        {
            ...
            return new LabelObject(newpos, news, smoothingMethod, closed);
        }
        Vec3 v[] = new Vec3 [vertex.length];
        for (int i = 0; i < v.length; i++)
            v[i] = new Vec3(vertex[i].r);
    ...
    /** Return a new Label object which has been subdivided the specified number of times to give a finer approximation of
     the curve shape. */
    
    public LabelObject subdivideLabel(int times)
    {
        LabelObject c = this;
    ...
    /** The following two routines are used by subdivideLabel to calculate new point positions
    ...
    {
        TriangleMesh mesh = triangulateLabel();
        if (mesh != null)
            mesh = TriangleMesh.optimizeMesh(mesh);
        return mesh;
    }
    
    private TriangleMesh triangulateLabel()
    {
        //System.out.println("  LabelObject.triangulateLabel() ");
        ...
        // Find the largest Label of the line, and project the vertices onto the other two.
    ...
    public LabelObject(DataInputStream in, Scene theScene) throws IOException, InvalidObjectException
    {
        super(in, theScene);
        
        //System.out.println("  LabelObject( instream, scene ) ");
        ...
        in.readFully(data);
        labelText = new String(data, "UTF-8");
    }
    ...
    public void renderObject(ObjectInfo obj, ViewerCanvas canvas, Vec3 viewDir)
    {
        ...
        if (mesh != null)
        {
            ...
            // Draw Label mark and annotation.
            canvas.renderLabelObject( obj, theCamera );
    ...
    // ??? how to designate this object is a Label???
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException
    {
        super.writeToFile(out, theScene);
        
        //System.out.println("  LabelObject writeToFile ( outstream, scene ) ");
        ...
        out.writeInt(smoothingMethod); // maybe use this method ID as a designation for Label object
        
        // Write label to the stream
        byte[] data = labelText.getBytes("UTF-8");
    ...
    {
        LabelKeyframe key = (LabelKeyframe) k;
    ...
    public static class LabelKeyframe extends MeshGesture
    {
        Vec3 vertPos[];
        float vertSmoothness[];
        LabelObject curve;
        
        public LabelKeyframe(LabelObject curve)
        {
            this.curve = curve;
        ...
        {
            LabelKeyframe k = new LabelKeyframe();
            if (owner instanceof LabelObject)
                k.curve = (LabelObject) owner;
            else
                k.curve = (LabelObject) ((ObjectInfo) owner).getObject();
        ...
        public void blendSurface(MeshGesture average, MeshGesture p[], double weight[])
        {
            ...
            LabelKeyframe avg = (LabelKeyframe) average;
            for (int i = 0; i < weight.length; i++)
            {
                LabelKeyframe key = (LabelKeyframe) p[i];
        ...
        public LabelKeyframe(DataInputStream in, Object parent) throws IOException, InvalidObjectException
        {
            this();
            short version = in.readShort();
            if (version != 0)
                throw new InvalidObjectException("");
            curve = (LabelObject) parent;
