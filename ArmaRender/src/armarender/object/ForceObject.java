...
public class ForceObject extends Curve implements Mesh
{
    ...
    public ForceObject(Vec3 v[], float smoothness[], int smoothingMethod, boolean isClosed)
    {
    ...
    public Object3D duplicate()
    {
        ...
        return new ForceObject(v, s, smoothingMethod, closed);
    }
    
    public void copyObject(Object3D obj)
    {
        ForceObject cv = (ForceObject) obj;
    ...
    public ForceObject subdivideForce()
    {
        //System.out.println("  ForceObject.subdivideForce() ");
        if (vertex.length < 2)
            return (ForceObject) duplicate();
        if (vertex.length == 2)
        {
            ...
            return new ForceObject(newpos, news, smoothingMethod, closed);
        }
        Vec3 v[] = new Vec3 [vertex.length];
        for (int i = 0; i < v.length; i++)
            v[i] = new Vec3(vertex[i].r);
    ...
    public ForceObject subdivideForce(int times)
    {
        ForceObject c = this;
    ...
    {
        //System.out.println("  ForceObject.triangulateForce() ");
    ...
    public ForceObject(DataInputStream in, Scene theScene) throws IOException, InvalidObjectException
    {
        super(in, theScene);
        
        //System.out.println("  ForceObject( instream, scene ) ");
    ...
    public void renderObject(ObjectInfo obj, ViewerCanvas canvas, Vec3 viewDir)
    {
        ...
        if (mesh != null)
        {
            ...
            canvas.renderForceObject( obj, theCamera );
    ...
    public void writeToFile(DataOutputStream out, Scene theScene) throws IOException
    {
        ...
        //System.out.println("  ForceObject writeToFile ( outstream, scene ) ");
    ...
    public static class ForceKeyframe extends MeshGesture
    {
        ...
        public ForceKeyframe(ForceObject curve)
        {
            this.curve = curve;
        ...
        public Keyframe duplicate(Object owner)
        {
            ...
            if (owner instanceof ForceObject)
                k.curve = (ForceObject) owner;
            else
                k.curve = (ForceObject) ((ObjectInfo) owner).getObject();
