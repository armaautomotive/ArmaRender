...
public class CSGModeller
{
  ...
  // JDT handler for existing function.
    public CSGModeller(TriangleMesh obj1, TriangleMesh obj2, CoordinateSystem coords1, CoordinateSystem coords2)
    {
        init(obj1, obj2, coords1, coords2, "");
    }
    
    
  public CSGModeller(TriangleMesh obj1, TriangleMesh obj2, CoordinateSystem coords1, CoordinateSystem coords2, String object_info)
  {
    init(obj1, obj2, coords1, coords2, object_info);
  }
    
    public void init(TriangleMesh obj1, TriangleMesh obj2, CoordinateSystem coords1, CoordinateSystem coords2, String object_info){
        //System.out.println("CSGModeller");
        ...
        //System.out.println("b " + obj1.getVertices().length + " " + obj2.getVertices().length );
        ...
        //System.out.println("e   vert1 " + vert1.size() + " face " + face1.size()  + "  vert2 " + vert2.size() + " face2 " + face2.size() ) ;
        // Step 1: Split the faces of each object so that they do not intersect.
        
        splitFaces(vert1, face1, bounds1, vert2, face2, bounds2, object_info);
        //System.out.println("e1   vert2 " + vert2.size() + " face2 " + face2.size() +  " vert1 " + vert1.size() + " face1 " + face1.size()  );
        splitFaces(vert2, face2, bounds2, vert1, face1, bounds1, object_info);
  ...
  public TriangleMesh getMesh(int op, Texture texture)
  {
    ...
    //System.out.println(" size " + vert1.size() + " " + vert2.size());
  ...
  private void splitFaces(Vector<VertexInfo> v1, Vector<FaceInfo> f1, BoundingBox bounds1, Vector<VertexInfo> v2, final Vector<FaceInfo> f2, BoundingBox bounds2, String object_info)
  {
    ...
    minAfter[f2.size()-1] = f2.get(faceIndex[f2.size()-1]).min;
      ...
      //System.out.println("limit " + limit + "  size 1: " + f1.size()  +"  2: "+ f2.size());
      //System.out.println("");
      System.out.println("  part 1: " + f1.size()  +"  2: "+ f2.size());
      
      int scans = 0;
      boolean running = true;
      
    p1 :for (int i = 0; i < f1.size() && running; i++)
    {
      ...
      for (int j = start; j < f2.size() && running; j++)
      {
        ...
        if (!intersect(fa.bounds, fb.bounds))
          ...
          scans++;
          //System.out.print(" i " + i + " j " + j + " " + scans + " " );
          if(scans > limit + 1){
              System.out.println(" *** INFINITE LOOP: " + object_info + " *** ");
              //i = f1.size();
              //j = f2.size(); // break
              running = false;
          }
