    ...
    if (dragPoint.x == clickPoint.x || dragPoint.y == clickPoint.y)
      {
        ((SceneViewer) view).repaint();
          ...
          {
            ObjectInfo selInfo = ((LayoutWindow) theWindow).getScene().getObject(sel[i]);
            view.getCamera().setObjectTransform(selInfo.getCoords().fromLocal());
            Rectangle bounds;
            bounds = view.getCamera().findScreenBounds(selInfo.getBounds());
            if (!selInfo.isLocked() && bounds != null && !selInfo.isChildrenHiddenWhenHidden() && pointInRectangle(clickPoint, bounds))
            {
    ...
    int sel[] = ((LayoutWindow) theWindow).getSelectedIndices();
      ...
      if(sel.length == 1){
          ...
          if(existingSelectedInfo != null
             ...
             (existingSelectedInfo.getObject() instanceof Curve == false)  // not curves
            ){
