...
public class FiveAxisFinishing extends Thread {
     ...
     * getUserInput
     *
     * Description: Promt user for parameters to use when exporting.
     */
    public boolean getUserInput(){
        JPanel panel = new JPanel();
        //panel.setBackground(new Color(0, 0, 0));
        panel.setSize(new Dimension(390, 600));
     ...
     */
    public void run(){
        //exportRoughingGCode(); // moved to seperate file.
     ...
     * TODO: Keep dialog forfront
     */
    public void exportFinishingGCode(){
        System.out.println("Export Five Axis 3D Mill Finishing GCode.");
        
        final JDialog progressDialog = new JDialog(); //  parentFrame , "Progress Dialog", true ); // parentFrame , "Progress Dialog", true); // Frame owner
        //progressDialog.setModal(true); // breaks app
        ...
        if(sceneName == null){
            // Error, can only export saved files.
            ImageIcon iconImage = new ImageIcon(getClass().getResource("/armarender/Icons/favicon-32x32.png"));
            JOptionPane.showMessageDialog(null, "Only saved project files can be exported.",  "Error" , JOptionPane.ERROR_MESSAGE, iconImage);
            return;
        }
        ...
        // Finishing sections are always 1.
        int sections = 1;
        /*
        ...
        if(simulateRoute){
            ...
            // TODO: Look through scene for object named after tool bit: T0 - T10
        ...
        for(int x = 0; x < mapWidth + 1; x++){
            for(int z = 0; z < mapDepth + 1; z++){
                ...
                for (ObjectInfo obj : objects){
                    if(
                       obj.getName().indexOf("Camera") < 0 && // DirectionLight SceneCamera
        ...
        for(int s = 0; s < sections && running; s++){ // height sections
            ...
            gcode += "; 5 Axis CNC Mill Finish Pass\n";
            gcode += "; Nozzle distance: ["+nozzleDistance+"]\n";
            ...
            for(int x = 0; x <= mapWidth && running; x++){
                ...
                for(; z <= mapDepth && skipZ == false && running; z++ ){ // && z >= zstart
                    ...
                    Vec3 normal = mapNormals[x][alternatingZ];
                    Vec3 unfilteredNormal = new Vec3(normal);
                    ...
                    if( angleBetweenNormalAndVertical < 0.5 ){ // If the angle of the drill bit is less than X, don't accomodate, C can fluctuate wildly.
                        normal = vertical;
                    }
                    ...
                    */
                    ...
                    double bValue = Math.toDegrees( fifthAxis ); // cValue -= 90; // rotate
                    double cValue = Math.toDegrees( fourthAxis ) - 180;
                    
                    
                    boolean ignoreCurrentInstruction = false;
                    double diff = Math.abs(cValue - previousCValue);
                    //System.out.println("C Value " + cValue + " prev " + previousCValue + "   diff: " + diff + "");
                    if( diff > 50 ){ // If difference in C value is too high then, handle.
                        double unfilteredNormalAngleToVertical = unfilteredNormal.getAngle(new Vec3(0, 1, 0));
                        unfilteredNormalAngleToVertical = Math.toDegrees(unfilteredNormalAngleToVertical);
                        
                        System.out.println("Error: C Value " + cValue + " prev " + previousCValue + "   diff: " + diff + " "  + "  angle " + unfilteredNormalAngleToVertical);
                        //System.out.println("C Value " + cValue + " prev " + previousCValue);
                    ...
                    //
                    // Translate C around origin
                    //
                    cValue += cAngleOrigin;
                    
                    
                    // G1 X.125 Y6.125 Z3 B0 C84.147 F40;
                    // G1 X.125 Y6.064 Z3 B0 C-254.37 F40;
                    
                    if(ignoreCurrentInstruction == false){
                    ...
                    if(simulateRoute && ignoreCurrentInstruction == false){
                        ...
                        // Update data in progress:
                        //progressLabel.setText("C: " + roundThree(cValue) + " B: " + roundThree(bValue) );
            ...
            try {
                String gcodeFile = dir + System.getProperty("file.separator") + "finishing_" + s + ".gcode";
                //gcodeFile += ".gcode";
        ...
        if(simulateRoute){
            if(drillTipCube != null){
                //window.getScene().removeObjectInfo(drillTipCube);
                window.removeObjectL(drillTipCube);
            }
            if(abPivotAvatarCube != null){
                //window.getScene().removeObjectInfo(abPivotAvatarCube);
                window.removeObjectL(abPivotAvatarCube);
            }
            if(extruderCurve != null){
                //window.getScene().removeObjectInfo(extruderCurve);
                window.removeObjectL(extruderCurve);
            }
            if(extruderCubeAvatar != null){
                //window.getScene().removeObjectInfo(extruderCubeAvatar);
