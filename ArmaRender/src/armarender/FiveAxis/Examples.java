...
public class Examples {
    ...
    public Vector<SurfacePointContainer> calculateFinishingRoutingPassWithBC( LayoutWindow window,
        ...
        // Take data that are not only used for router geometry
        double cutTipToColletLength =  getDoubleProperty(bitProps, "ads.export_mill_5axis_bit_cuttip_to_collete_length", 7);
        double fulcrumToToolTipLength = getDoubleProperty(routerConfigProps, "ads.export_mill_5axis_collet_distance", 7.5) + cutTipToColletLength;
        ...
        constructRouterGeometry(window, firstRegionSurfacePoint, routerElements, b, c, ballNoseTipType); // Construct Router 3D Model
    ...
    public String toolPathToGCode(LayoutWindow window, Vector<SurfacePointContainer> generatedCuttingPath, double speed){
        ...
        for(int i = 0; i < generatedCuttingPath.size(); i++){
            ...
            // speed
            speed = getDoubleProperty(routerConfigProps, "ads.export_5axis_speed", 50);
            if( i > 2 && i < generatedCuttingPath.size() - 6 ){
                Vec3 oppositeP = generatedCuttingPath.elementAt(i - 1).point;
                Vec3 oppositeP2 = generatedCuttingPath.elementAt(i - 2).point;
                Vec3 prevPointA = generatedCuttingPath.elementAt(i + 1).point;
    ...
    //
    // This includes multiple objects that represent the router machine that are used to detect collisions.
    ...
    //
    public void constructRouterGeometry(LayoutWindow window, Vec3 firstRegionSurfacePoint, Vector<RouterElementContainer> routerElements, double b, double c, boolean ballNoseTipType){
        Scene scene = window.getScene();
    ...
    public String getStringProperty(Properties prop, String property, String defaultVal) {
        String value = prop.getProperty(property);
