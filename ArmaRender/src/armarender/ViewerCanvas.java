...
public abstract class ViewerCanvas extends CustomWidget
{
  ...
  private static boolean vulcanAvailable;
  ...
  public Vec3 viewTarget = null;
    ...
    static
    {
      try
      {
          Class.forName("org.lwjgl.vulkan.VkDevice");
          vulcanAvailable = true;
          System.out.println("Vulkan Present.");
      }
      catch (Throwable t)
      {
        System.out.println("Error loading Vulkan class: "+t);
        System.out.println("java.library.path: "+System.getProperty("java.library.path"));
      }
    }
     */
    
    // OpenGL is depricated.
    /*
  static
  {
    try
  ...
  public static RGBColor replicatedTransparentColor;
  ...
  public ViewerCanvas()
  {
    this(ArmaRender.getPreferences().getUseOpenGL() && openGLAvailable);
  }
  ...
  public ViewerCanvas(boolean useOpenGL)
  {
    ...
    //setBackground(backgroundColor);
      ...
      replicatedTransparentColor = new RGBColor(0.74, 0.94, 0.74);
  ...
  protected void buildChoices(RowContainer row)
  {
    for (int i = 0; i < controls.size(); i++)
    {
      ...
      if (w != null)
      {
          // view canvas elements are added in the ArmaRender class to be applied to all but some are filtered out bere by position.
          // If view
          if( this.position != 0 && controls.get(i) instanceof armarender.view.ViewerFrontButtonControl){
          } else if( this.position != 1 && controls.get(i) instanceof armarender.view.ViewerLeftButtonControl){
          } else if( this.position != 2 && controls.get(i) instanceof armarender.view.ViewerTopButtonControl){
          } else {
              row.add(w);
  ...
  private void processMouseReleased(WidgetMouseEvent ev)
  {
     ...
     *
     * Description: handle mouse scroll movements. Adjust scale for parallel and camera location for perspective.
     */
  protected void processMouseScrolled(MouseScrolledEvent ev)
  {
    ...
    if (ArmaRender.getPreferences().getReverseZooming())
      amount *= -1;
    ...
    {
        // Set view scale from scenecamera if applicable
        ...
        // Save new scroll value in sceneCamera object if applicable.
        if (boundCamera != null && boundCamera.getObject() instanceof SceneCamera){
            SceneCamera sc = (SceneCamera)boundCamera.getObject();
            sc.setScale(getScale()); // Camera
        }
    }
  }
  ...
  public void setPerspective(boolean perspective)
  {
    ...
    repaint();
      ...
      // If the selected view is a camera, save the perspective/parallel when we change it from the drop down.
      if (boundCamera != null && boundCamera.getObject() instanceof SceneCamera){
          SceneCamera sc = (SceneCamera)boundCamera.getObject();
          sc.setPerspective( perspective );
          
          //viewChanged(true); // causes propertie to be updated.
      }
  }
  
  /**
   * isPerspective
   * Description: Determine whether the view is currently is perspective mode.
   * This function is confusing. The view has a drop down menu for perspective, and a selected camera object has a setting.
   *
   */
  public boolean isPerspective()
  {
    //if (boundCamera != null && boundCamera.getObject() instanceof SceneCamera){ // if view selection is camera, use its perspective setting.
  ...
  {
      //System.out.println("ViewerCanvas setOrientation " + which );
    ...
    theCamera.setCameraCoordinates(coords);
      ...
      // If a target exists, set view target.
      if(viewTarget != null){
          setViewTarget(which, viewTarget);
          viewTarget = null;
      }
     ...
     * setViewTarget
     *
     * Description: Move camera location to centre on target point (usually a selection).
     *  Called by SceneViewer.setOrientation() when a selection exists.
     *
     *
     */
    public void setViewTarget(int which, Vec3 targetPoint){
        CoordinateSystem coords = theCamera.getCameraCoordinates();
        Vec3 caneraOrigin = coords.getOrigin();
        if(orientation == 0){ // Front
            caneraOrigin.x = targetPoint.x;
            caneraOrigin.y = targetPoint.y;
            coords.setOrigin(caneraOrigin);
        }
        if (which == 1){        // Back
            caneraOrigin.x = targetPoint.x;
            caneraOrigin.y = targetPoint.y;
    ...
    public void setViewTargetPoint(Vec3 targetPoint){
        viewTarget = targetPoint;
    }
  ...
  */
  ...
  public void renderLabelObject(ObjectInfo obj, Camera theCamera)
  {
    ...
    public void renderArcObject(ObjectInfo obj, Camera theCamera){
        drawer.renderArcObject(obj, theCamera);
    }
    
    public void renderForceObject(ObjectInfo obj, Camera theCamera)
    {
      drawer.renderForceObject(obj, theCamera);
    }
  ...
  public void renderMeshTransparent(RenderingMesh mesh, VertexShader shader, Camera cam, Vec3 viewDir, boolean hideFace[])
  {
    ...
    {
      drawer.renderMeshTransparent2(mesh, shader, cam, viewDir, hideFace, closed);
    }
