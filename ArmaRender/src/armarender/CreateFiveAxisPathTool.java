...
/** CreateArcTool is an EditingTool used for creating Curve objects. */

public class CreateFiveAxisPathTool extends EditingTool
{
  static int counter = 1;
  private Vector<Vec3> clickPoint;
  private Vector<Float> smoothness;
  private int smoothing;        // depricate
  private FiveAxisPath theCurve; //
  private CoordinateSystem coords;
    
  private double bendRadius = 0.1441;
  private double tubeDiameter = 0;
  private double tubeWall = 0;

  public static final int HANDLE_SIZE = 3;

  public CreateFiveAxisPathTool(EditingWindow fr)
  {
    super(fr);
    initButton("5AxisPath_48"); // 5AxisPath_48
    smoothing = Mesh.NO_SMOOTHING; //  Mesh.APPROXIMATING;
  }

  public void activate()
  {
    super.activate();
    theWindow.setHelpText(Translate.text("createFiveAxisPathTool.helpText"));
  }
  ...
  {
      return "Create five axis tool path."; // Translate.text("createCurveTool.tipText");
  }
    ...
    if (clickPoint.size() > 1)
      {
        ...
        theCurve = new FiveAxisPath(vertex, s, smoothing, false);
        theCurve.setBendRadius(bendRadius);
        theCurve.setTubeDiameter(tubeDiameter);
        theCurve.setTubeWall(tubeWall);
    ...
    if (addCurve)
      {
        ...
        ObjectInfo info = new ObjectInfo(theCurve, coords, "5-Axis Path "+(counter++));
        info.addTrack(new PositionTrack(info), 0);
        info.addTrack(new RotationTrack(info), 1);
          
          
          theCurve.setScene(((LayoutWindow)theWindow).getScene() ); // Arc needs the scene to get texture objects for drawing cylinders.
   ...
   * iconDoubleClicked
   *
   * Description: Collect arc properties (radius of die, tube diameter, tube wall thickness).
   */
  public void iconDoubleClicked()
  {
    ...
    BTextField bendRadiusField = new BTextField("0.1441");
    BTextField tubeDiameterField = new BTextField("0.0480359");
    BTextField tubeWallField = new BTextField("0.0022817");
    ...
    if (smoothing == Mesh.INTERPOLATING)
      ...
      ComponentsDialog dlg = new ComponentsDialog(theFrame, Translate.text("Five Axis Path Properties"), // selectCurveSmoothing
                new Widget [] {bendRadiusField, tubeDiameterField, tubeWallField},
                new String [] { "Bend Radius", "Tube Diameter", "Tube Wall"} // Translate.text("Smoothing Method"),
                );
      
    if (!dlg.clickedOk())
      return;
    bendRadius = Double.parseDouble(bendRadiusField.getText());
    tubeDiameter = Double.parseDouble(tubeDiameterField.getText());
    tubeWall = Double.parseDouble(tubeWallField.getText());
    /*
