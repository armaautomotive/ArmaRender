...
public class ObjectPropertiesPanel extends ColumnContainer
{
  ...
  protected void rebuildContents()
  {
    ...
    Vec3 origin = objects[0].getCoords().duplicate().getOrigin();
      
      // Multiple by scale factor
      //origin.x *= scene.getScale();
      //origin.y *= scene.getScale();
      //origin.z *= scene.getScale();
    ...
    LayoutInfo fillLayout = new LayoutInfo(LayoutInfo.CENTER, LayoutInfo.HORIZONTAL, new Insets(2, 2, 2, 2), null);
      //fillLayout.setBackground(new Color(255, 0, 0));
    if (objects.length == 1)
    {
    ...
    FormContainer positions = new FormContainer(new double [] {0, 1, 0, 1, 0, 1}, new double [1]);
      positions.setBackground(new Color(220, 220, 220));
    ...
    FormContainer orientation = new FormContainer(new double [] {0, 1, 0, 1, 0, 1}, new double [1]);
      orientation.setBackground(new Color(220, 220, 220));
    ...
    for (int i = 0; i < propEditor.length; i++)
    {
      ...
      widget.setBackground(new Color(220, 220, 220));
  ...
  private void coordinatesChanged(ValueChangedEvent ev)
  {
    ...
    for (int i = 0; i < objects.length; i++)
    {
      ...
      coords.setOrientation(angles[0], angles[1], angles[2]);
        ...
        //
        ObjectInfo parent = objects[i].getParent();
        if(parent != null){
            ...
            if(parentObj instanceof Folder){
                ...
                if(f.getAction() == Folder.UNION || f.getAction() == Folder.SUBTRACT || f.getAction() == Folder.INTERSECTION){
                    if(window instanceof LayoutWindow){
                        ...
                        if(rootObject != null){
                            ((LayoutWindow)window).updateBooleanObject( rootObject ); // regenerate boolean objects affected by this change.
                        }
                    }
                }
            }
      ...
      for (int j = 0; j < propEditor.length; j++)
        if (propEditor[j].getWidget() == ev.getSource())
        {
          ...
          if (!objects[i].getObject().getPropertyValue(j).equals(value))
          {
              ...
              // If object is ArcTube and it has a mirror plane child, replicate property changes to mirrored objects.
              if(objects[i].getObject() instanceof Curve || objects[i].getObject() instanceof Mesh){ // ArcObject
                  ObjectInfo [] children = objects[i].getChildren();
                  for(int c = 0; c < children.length; c++){
                      ...
                      if(chlid.getObject() instanceof MirrorPlaneObject){
                          ...
                          for(int d = 0; d < mirrored.length; d++){
                              ...
                              if(replicated.getObject() instanceof Curve || objects[i].getObject() instanceof Mesh){ // ArcObject
                                  replicated.getObject().setPropertyValue(j, value);
