...
import armadesignstudio.animation.PositionTrack;
import armadesignstudio.animation.RotationTrack;
...
public class CreateProceduralImplicitObjectTool extends EditingTool {
	...
	if (CreateProceduralImplicitObjectTool.icon == null)
	    CreateProceduralImplicitObjectTool.icon = new ImageIcon(
		    getClass().getResource(
			    "/armadesignstudio/implicit/Icons/procedural.gif"))
		    .getImage();
	if (CreateProceduralImplicitObjectTool.selectedIcon == null)
	    CreateProceduralImplicitObjectTool.selectedIcon = new ImageIcon(
		    getClass()
			    .getResource(
				    "/armadesignstudio/implicit/Icons/selected/procedural.gif"))
		    .getImage();
    }
	...
	theWindow.setHelpText(IPTranslate
		.text("createProceduralObjectTool.helpText"));
	...
	if (shiftDown) {
	    if (Math.abs(dragPoint.x - clickPoint.x) > Math.abs(dragPoint.y
		    - clickPoint.y)) {
	...
	v2 = cam.convertScreenToWorld(new Point(dragPoint.x, clickPoint.y),
		ModellingApp.DIST_TO_SCREEN);
	...
	ObjectInfo info = new ObjectInfo(obj, new CoordinateSystem(orig, zdir,
		ydir), "ProceduralImplicitObject "
		+ (CreateProceduralImplicitObjectTool.counter++));
	info.addTrack(new PositionTrack(info), 0);
	info.addTrack(new RotationTrack(info), 1);
	UndoRecord undo = new UndoRecord(theWindow, false);
	undo.addCommandAtBeginning(UndoRecord.SET_SCENE_SELECTION,
		new Object[] { theScene.getSelection() });
