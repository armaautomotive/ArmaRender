     ...
     implements Plugin
{
    ...
    public void processMessage( int message, Object args[] )
    {
        ...
        else if ( message == Plugin.SCENE_WINDOW_CREATED )
        {
            ...
            BMenu toolsMenu = layout.getToolsMenu(); //  menuBar.getChild( 4 ); // was 2. Grabbing a menu bar by index breaks when menu changes.
            int count = toolsMenu.getChildCount();
            //BMenu createMenu = (BMenu) toolsMenu.getChild(count-1); // Grabbing blingly the last toolMenu item.  BMenuItem cant be cast
	...
	private void doCreate()
        {
            ...
            if (obj.canSetTexture())
                info.setTexture(theScene.getDefaultTexture(), theScene.getDefaultTexture().getDefaultMapping( obj )); // Error
            
            // TextureMapping
            // Obj setTexture(Texture tex, TextureMapping map)
