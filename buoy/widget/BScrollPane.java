/**
 * Modifications: 2022, 2023, Arma Automotive Inc.
 *
 */
package buoy.widget;
...
public class BScrollPane extends WidgetContainer
{
  ...
  private JViewport rowHeaderPort, colHeaderPort, colFooterPort;
  private Widget content, rowHeader, colHeader, colFooter;
    private Widget upperRightCorner;
    private JViewport upperRightCornerPort;
  ...
  private boolean forceWidth, forceHeight;
    ...
    private ColumnContainer footerColumnContainer; // hscroll, colFooter
  ...
  {
      footerColumnContainer = new ColumnContainer(); // hscroll, colFooter
      //footerColumnContainer.setDefaultLayout(new LayoutInfo(LayoutInfo.WEST, LayoutInfo.BOTH, null, null));
      footerColumnContainer.getComponent().setLayout(new BorderLayout());
      footerColumnContainer.setBackground(Color.RED);
      
    JScrollPane panel = createComponent(); // ScrollPaneComponent
    panel.setBorder(BorderFactory.createEmptyBorder()); // removes line
    component = panel;  // Widget.component
    ...
    MouseWheelListener wheelListener = new MouseWheelListener() {
      public void mouseWheelMoved(MouseWheelEvent ev)
      {
        ...
        //int direction = (ev.getWheelRotation() > 0 ? 1 : -1); // // ERROR: Direction is allways -1 on MacOSX
    ...
    vscroll.getComponent().addMouseWheelListener(wheelListener);
      ...
      //footerColumnContainer.add(hscroll);
      panel.add(hscroll.getComponent());
      //panel.add(footerColumnContainer.getComponent());
    ...
    panel.setColumnHeader(colHeaderPort = new JViewport());
      ...
      panel.setCorner(JScrollPane.UPPER_RIGHT_CORNER, upperRightCornerPort = new JViewport() ) ;
      
      upperRightCornerPort.setBackground(Color.BLUE);
      
      
      // Paint the upper right corner
      CustomWidget cwCorner = new CustomWidget();
      cwCorner.setPreferredSize(new Dimension(10, 10));
      cwCorner.setBackground(Color.BLUE);
      //panel.setCorner(ScrollPaneConstants.LOWER_RIGHT_CORNER, cwCorner.getComponent()); // no efect
      
      
      upperRightCorner = cwCorner;

      
      colFooterPort = new JViewport(); // depricate
      //panel.setColFooter();
      //panel.add(colFooter.getComponent());
      
    rowHeaderPort.setLayout(null);
    colHeaderPort.setLayout(null);
    rowHeaderPort.setOpaque(false);
    colHeaderPort.setOpaque(false);
      colFooterPort.setLayout(null);
      colFooterPort.setOpaque(false);
      upperRightCornerPort.setLayout(null);
      upperRightCornerPort.setOpaque(false);
  ...
  public void setColHeader(Widget widget)
  {
    ...
    // Experimental
    public void setCorner(Widget widget){
        // upperRightCornerPort
        // upperRightCorner
        
        if (upperRightCorner != null)
          removeAsParent(upperRightCorner);
        upperRightCorner = widget;
        if (upperRightCorner != null)
          setAsParent(upperRightCorner);
        upperRightCornerPort.setView(upperRightCorner == null ? null : upperRightCorner.getComponent());
        invalidateSize();
    }
     ...
     * ????
     */
    public void setColFooter(Widget widget)
    {
      if (colFooter != null)
        removeAsParent(colFooter);
        colFooter = widget;
      if (colFooter != null)
        setAsParent(colFooter);
      //colFooterPort.setView(colFooter == null ? null : colFooter.getComponent());
      
        //getComponent().add(widget.getComponent());
        //footerColumnContainer.add(widget.getComponent(), BorderLayout.SOUTH);

        //widget.getComponent().setMaximumSize(new Dimension(10000, 20));
        //widget.getComponent().setPreferredSize(new Dimension(600, 20));
        //widget.getComponent().setMinimumSize(new Dimension(1, 20));
  ...
  {
    int topMargin = 0, leftMargin = 0, bottomMargin = 0, rightMargin = 0, bottomFooterMargin = 0;
    ...
    Dimension colFooterSize = (colFooter == null ? null : colFooter.getPreferredSize());
    ...
    if(colFooterSize != null){
        //bottomFooterMargin = colFooterSize.height;
        bottomFooterMargin = 20;
    }
    ...
    Rectangle viewBounds = new Rectangle(leftMargin, topMargin, bounds.width-leftMargin-rightMargin, bounds.height-topMargin-bottomMargin - bottomFooterMargin);
    
      //System.out.println("viewBounds: " + viewBounds.width + " " + viewBounds.height);
    ...
    if (colHeader != null)
    {
      ...
      colHeader.getComponent().setSize(size);
        ...
        //System.out.println("colHeader " + colHeaderPort.getBounds() );
    }
    ...
    if (rowHeader != null)
    {
      ...
      rowHeader.getComponent().setSize(size);
        ...
        //System.out.println("rowHeader " + size);
    }
    ...
    vscroll.setExtent(viewBounds.height); //+  bottomFooterMargin
    
      //System.out.println("Scroll: " + vscroll.getComponent().getBounds());
      
      
      // X, Y, W, H
      upperRightCornerPort.setBounds(  new Rectangle(viewBounds.x+viewBounds.width, topMargin - 5, rightMargin, vScrollLength) );
      if (content == null)
          upperRightCornerPort.setEnabled(false);
      else
      {
          //upperRightCornerPort.setEnabled(true);
          //int width = content.getComponent().getWidth();
          //upperRightCornerPort.setMaximum(width);
          //if (upperRightCornerPort.getValue()+viewBounds.width > width)
          //  upperRightCornerPort.setValue(width-viewBounds.width);
          
          
          Dimension size = new Dimension(10, 10);
          //if (forceHeight)
          //size.height = forceSize.height;
          upperRightCorner.getComponent().setSize(size);
      }
      //upperRightCornerPort.setExtent(viewBounds.width);
    ...
    if (rowHeader instanceof WidgetContainer)
      ...
      if(upperRightCorner instanceof WidgetContainer){
          ((WidgetContainer) upperRightCorner).layoutChildren();
      }
      
    if (colFooter instanceof WidgetContainer)
      ((WidgetContainer) colFooter).layoutChildren();
  }
  ...
  private class ScrollPaneComponent extends JScrollPane
  {
    ...
    public void paintComponent(Graphics g)
    {
      ...
      if (BScrollPane.this.isOpaque())
      {
        ...
        //g.drawLine(0, 2, 100, 15); //
          
          
          
          // if colFooter paint component
          /*
          if(colFooter != null){
              Dimension s = getViewSize();
              colFooter.getComponent().setBackground(Color.GREEN);
              //colFooter.getComponent().setPreferredSize(new Dimension(40, 40));
              //colFooter.getComponent().setBounds(new Rectangle(1, (int)(s.getHeight() - 10), (int)(s.getWidth()), 300));
              colFooter.getComponent().setBounds(new Rectangle(0, 0, (int)(s.getWidth()), 300));
              colFooter.getComponent().paint(g);
              //System.out.println("PAINT FOOTER BAR Component " + s.getWidth() + " " + s.getHeight());
          }
           */
