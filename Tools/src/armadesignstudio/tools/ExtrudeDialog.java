...
public class ExtrudeDialog extends BDialog
{
  ...
  private BRadioButton pathBox, xBox, yBox, zBox, vectorBox, normalsBox;
  ...
  public ExtrudeDialog(LayoutWindow window)
  {
    ...
    content.add(normalsBox = new BRadioButton("Normals", true, pathGroup), 0, 6);
      
    content.add(pathBox = new BRadioButton("Curve", true, pathGroup), 0, 7);
    content.add(vectorBox = new BRadioButton("Vector", true, pathGroup), 0, 8);
    ...
    content.add(pathChoice = new BComboBox(), 1, 7);        // 6
    ...
    content.add(orientBox = new BCheckBox("Orientation Follows Curve", true), 0, 9, 2, 1); // 8
    ...
    content.add(preview = new ObjectPreviewCanvas((ObjectInfo) objects.elementAt(0))  , 2, 3, 2, 6,
      ...
      preview.setPreferredSize(new Dimension(320, 280));
      
    RowContainer buttons = new RowContainer();
    content.add(buttons, 0, 10, 4, 1, new LayoutInfo()); // 0 9
  ...
  {
    distField.setEnabled(xBox.getState() || yBox.getState() || zBox.getState() || normalsBox.getState());
    ...
    if (profile.getObject() == path)
      obj = null;
        ...
        //} else {
            // Normals Extrude
        //    System.out.println(".");
        //    obj = extrudeNormalsMesh((TriangleMesh) profile.getObject(), path, profile.getCoords(), pathCoords, angleField.getValue()*Math.PI/180.0, orientBox.getState());
        //}
      ...
      obj = extrudeCurve((Curve) profile.getObject(), path, profile.getCoords(), pathCoords, angleField.getValue()*Math.PI/180.0, orientBox.getState());
        ...
        if(normalsBox.getState() == false){
            ...
            if( profile.getObject() instanceof armadesignstudio.object.SplineMesh ){
                ...
                double extrudeDistance = distField.getValue();
                
                obj = extrudeNormalsMesh(profile,
                                         sm,
                                         profile.getObject().convertToTriangleMesh(tolField.getValue()),
                                         normVec,
                                         extrudeDistance,
                                         path,
                                         profile.getCoords(),
  ...
  */
  ...
  public static Object3D extrudeMesh(TriangleMesh profile, Curve path, CoordinateSystem profCoords, CoordinateSystem pathCoords, double angle, boolean orient)
  {
    ...
    */
    ...
    public static Object3D extrudeNormalsMesh(ObjectInfo profileOI,
